...
for i in bag:
    ...
    while gem and i >= gem[0][0]:
        ...
        # -를 해줌으로서 gem[0][1] 가장최소(즉 나중에 -해줄거니까 가장 비싼 보석)값을 넣어준다
				# 그럼 알아서 -를 해줬으니까 가장 비싼것들순으로 정렬이 된다.
        heapq.heappush(tmp, -gem[0][1])
        # gem리스트에 가장 최소값인 즉 방금전에 넣으거 빼기
        heapq.heappop(gem)
    # 만약 tmp 배열에 값이 있고 가방이 남았으면 가장 비싼거 부터 빼오기(훔치기)
		# 그리고 훔친것을 비워준다.
    if tmp:
        ans -= heapq.heappop(tmp)
		# 결국 tmp 에는 가방이 허용가능한 순대로 보석의 최대 갑어치 순으로 정렬이 되어있다
print(ans)
